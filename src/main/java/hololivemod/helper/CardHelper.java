...
public class CardHelper {
    ...
    public static ArrayList<String> CombinationsName = new ArrayList<>();
    public static ArrayList<String> CombinationsDescription = new ArrayList<>();
    public CardHelper(){}
    ...
    public static ArrayList<AbstractCard> GenerateMinionGroup(boolean upgraded){
        ...
        while(derp.size() != 3) {
            ...
            AbstractCard tmp;
            do{
                tmp = CardLibrary.getAnyColorCard(AbstractCard.CardType.POWER, AbstractCard.CardRarity.UNCOMMON);
            }while(!(tmp instanceof AbstractSummonCard));
    ...
    public static void OnChooseThisCard(AbstractCard card,int Index,int EffectTimes,int Effectnum,int MagicNum,int MagicNum2){
        ...
        for(j = 0 ; j < EffectTimes ; ++j){
            switch (Effectnum) {
                ...
                case 2:
                    if (minion instanceof AbstractMinion) {
                        ((AbstractMinion) minion).ChangeStat(MagicNum, MagicNum, false);
                ...
                case 7:
                    if(minion instanceof AbstractMinion) {
                        ((AbstractMinion) minion).ChangeStat(MagicNum, MagicNum2, false);
                    }
                    break;
                default:
                    break;
    ...
    public static ArrayList<AbstractCard> GetCombinationOptionCards() {
        boolean[] hasMinion = new boolean[43];
        ArrayList<AbstractCard> cards = new ArrayList<>();
        for (AbstractCard c : AbstractDungeon.player.masterDeck.group) {
            {
                if(c instanceof AbstractSummonCard){
                    ...
                    if (c instanceof CallYukihanaLamy) {
                        hasMinion[38] = true;
                        continue;
                    }
                    if (c instanceof CallShishiroBotan) {
                        hasMinion[39] = true;
                        continue;
                    }
                    if (c instanceof CallOmaruPolka) {
                        hasMinion[40] = true;
                        continue;
                    }
                    if (c instanceof CallManoAloe) {
                        hasMinion[41] = true;
                        continue;
                    }
                    if (c instanceof CallMomosuzuNene) {
                        hasMinion[42] = true;
                    }
                }
            }
        ...
        if(hasMinion[38] && hasMinion[39] && hasMinion[40] && hasMinion[41] && hasMinion[42] && !TheStar.CombinationIndex[26]) {
            cards.add(new FiveCombinationOption());
        }
        int hasall = 0;
