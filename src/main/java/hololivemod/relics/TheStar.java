...
public class TheStar extends CustomRelic {
   ...
   //    return damageAmount;
    ...
    public void onPlayCard(AbstractCard c, AbstractMonster m){
        ...
        if(CombinationIndex[26]){
            this.amount++;
            if(amount >= 5){
                this.flash();
                for(AbstractOrb o: AbstractDungeon.player.orbs){
                    if(o instanceof AbstractMinion){
                        ((AbstractMinion) o).AttackEffect();
                    }
                }
                amount = 0;
            }
    ...
    public void onGainGold(){
            if(AbstractDungeon.floorNum > 0 && AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT && !AbstractDungeon.getMonsters().areMonstersBasicallyDead()) {
    ...
    public void renderCounter(SpriteBatch sb, boolean inTopPanel) {
        ...
        if (this.amount > 0) {
            FontHelper.renderFontRightTopAligned(sb, FontHelper.topPanelInfoFont, Integer.toString(this.amount), this.currentX + 30.0F * Settings.scale, this.currentY - 21.0F * Settings.scale, Color.WHITE);
        }
    }
